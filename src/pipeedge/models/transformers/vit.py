...
PRUNED = True
PRUNED_WEIGHT_FILE = 'vit-pruned_original.npz'
...
class ViTModelShard(ModuleShard):
    ...
    def _build_shard(self, weights, pruned=PRUNED):
        if(pruned):
            weights = np.load(PRUNED_WEIGHT_FILE)
    ...
    @torch.no_grad()
    def _load_weights_first(self, weights, pruned=PRUNED):
        if(pruned):
    ...
    @torch.no_grad()
    def _load_weights_last(self, weights, pruned=PRUNED):
        if(pruned):
    ...
    @torch.no_grad()
    def _load_weights_layer(self, weights, layer_id, layer, pruned=PRUNED):
...
class ViTShardForImageClassification(ModuleShard):
    ...
    def __init__(self, config: ViTConfig, shard_config: ModuleShardConfig,
                 model_weights: Union[str, Mapping], prune=''):
        super().__init__(config, shard_config)
        if(prune == 'r'):
            PRUNED = True
        self.vit = None
    ...
    def _load_weights_last(self, weights, pruned=PRUNED):
        if(pruned):
            with np.load('vit-pruned_original.npz') as weights_pruned:
                # print(1)
    ...
    def prune_original(self, ubatch, ubatch_labels, keep_ratio = 0.1):
        ...
        model_file ='vit-pruned_original.npz'
